...
namespace DatabaseEssential
{
    public class DatabaseManager : MonoBehaviour
    {
        ...
        {
            string query = $"SELECT Count(*) FROM member where user_email='{email}' and user_pw='{password}'";
            int count = -1; // Negative value will represent the failure of connection
        ...
        {
            string query = $"SELECT * FROM member where user_email='{email}'";
            string temp ="";
            ...
            if (this.Initialize() == true)
            {
                ...
                while (dataReader.Read())
                {
                    ...
                    //    results[i].Add(dataReader[columns[i].ToString()] + "");
                    //}
                    temp = String.Format("{0}", dataReader["user_name"]);
                }
